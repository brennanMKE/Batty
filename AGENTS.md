...
This file is binding guidance for Codex sessions and subagents. **If something here contradicts a downstream skill, this file wins for code/repo conventions; `issues/Issues.md` wins for issue-tracker workflow.**
...
- **Resolving an issue is review-gated and role-split (binding, for cost and quality):**
  ...
  - A **filer subagent pinned to the top available model** (currently Fable, `Codex-fable-5`) creates new `NNNN.md` files on `main` — no branch, no review gate.
  - An **implementer subagent pinned to Sonnet** (`Codex-sonnet-5`) does the code work and verification on the branch.
  - A **reviewer subagent pinned to Opus** (`Codex-opus-4-8`) reviews `git diff main...HEAD`; approve or request-changes. Findings route back to the *same* implementer agent.
