...
- **Identity:** stable `id: UUID`, plus a user-editable `title` and a **Session color** — one of a fixed 10-color palette (`SessionColor`), assigned automatically at creation by a least-used round-robin scoped to the Window; that assignment is stable for the Session's lifetime unless the user pins a different one. Shown by tinting the Sidebar row's `rectangle.split.3x1` icon. The user can override the assignment via the Sidebar row's context menu (`Set Color…`, a swatch-grid picker; `Reset Color` returns the Session to automatic control without changing its current color, #0336). An explicit choice is sticky — no later automatic assignment reclaims it — and duplicate colors across Sessions are allowed. Not persisted: a Session's color, chosen or automatic, resets on relaunch (see Persisted, below).
...
- **Sidebar affordances:** drag to reorder; right-click to Rename / Duplicate / Set Color / Close; "+" toolbar button creates a new Session.
