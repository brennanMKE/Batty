...
nonisolated final class AppXPCService: NSObject, AppServiceProtocol {
    ...
    func perform(_ request: Data, reply: @escaping @Sendable (Data) -> Void) {
        ...
        case XPCVerb.status:
            ...
            // happen synchronously with `perform`'s own call.
            Task { @MainActor in
                let payload = AppStateStore.shared.statusPayload()
                guard let payloadData = try? JSONEncoder().encode(payload) else {
                    reply(Self.encode(XPCResponse(ok: false, error: "failed to encode status payload")))
                    return
                }
                ...
                reply(Self.encode(XPCResponse(ok: true, payload: payloadData)))
            }
        default:
            logger.error("perform: unknown verb \(decoded.verb, privacy: .public)")
            reply(Self.encode(XPCResponse(ok: false, error: "unknown verb: \(decoded.verb)")))
        }
    }
