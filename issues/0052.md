...
`tree.focusedPaneID` existed in the model but never reached the view layer. `PaneView` had no awareness of which pane in its containing session was the focused one, so every pane rendered identically: same chip palette, same terminal-area opacity, same border treatment. The per-pane `activeTabID` chip highlight (a tab-level signal) doubled as the only accent-colored chip in each pane and was being misread as a session-level focus signal.

## Fix

Threaded `SplitTree` from `SplitContainerView` down through `SplitNodeView` into `PaneView` so the leaf view can compute `isPaneFocused = tree.focusedPaneID == pane.id`. Three visual signals now reflect pane focus, gated on the session actually having more than one pane (a single-pane session has nothing to disambiguate):

- The focused pane's terminal-surface frame gains a 2pt accent-color border at 0.6 opacity.
- Non-focused panes' terminal-surface stacks render at 0.7 opacity. The opacity is applied to the terminal-surface `ZStack` only — not to the tab bar — so the unseen-bell dot in non-focused-pane chips stays at full salience.
- `BattyTabChip` gained an `isPaneFocused` parameter and now renders the active-tab background in a neutral gray on non-focused panes instead of accent. The unseen dot keeps its accent color regardless.

Transitions use `.easeInOut(duration: 0.12)` so focus moves feel responsive without flickering when the user cycles focus with `Cmd-Option-Arrow`.

## Files changed

- `BattyKit/Sources/BattyKit/SplitContainerView.swift` — `SplitNodeView` now holds `tree` as `@Bindable` and forwards it into `PaneView`.
- `BattyKit/Sources/BattyKit/PaneView.swift` — accepts `tree: SplitTree` in its initializer, computes `isPaneFocused` and `hasSiblingPanes`, applies the focus-border overlay and terminal-area dim with a 120ms ease-in-out animation, and passes `isPaneFocused` into `BattyTabChip`.
- `BattyKit/Sources/BattyKit/BattyTabChip.swift` — added `isPaneFocused` (defaults to `true` for back-compat) and a `showsActiveAccent` derived flag so the active-tab background/border use accent only when the pane is also focused; otherwise the active tab renders with a stronger gray treatment that still distinguishes it from inactive sibling tabs within the same pane.

## Gotchas

- The terminal-area dim is applied via `.opacity` on the terminal `ZStack`, deliberately not on the whole `PaneView`. If a future change moves the bell-flash or drag-hover overlays outside that stack and dims them too, the unseen-bell dot in the chip will also dim — which would defeat the "dot at full salience" acceptance criterion. Keep the dim local to the terminal surface.
- The signal is gated on `hasSiblingPanes` so a freshly opened single-pane session doesn't display a permanent accent border in its only pane. If a Session ever becomes empty mid-flight, `tree.allPanes.count` still reports the live count via `SplitTreeNode.allLeafPanes`.
