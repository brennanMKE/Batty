...
> 🟢 Resolved 2026-05-30 — Release tooling hardened so the marketing-version-bump and appcast-vs-DMG drifts that produced this dialog cannot recur. 1.0.3 is **not** being re-cut (see Decision); the fix is preventative, landing in 1.0.4 and after.

Root cause, the approach taken, the exact verification commands, and the files changed are detailed below under **Decision** and **Mitigation landed**. Code fix: `3197cb1`.
...
1.0.3 will **not** be re-cut. Re-cutting would also mean replacing the GitHub release artifact, which is messy for a build that's already in users' hands. We accept the mislabeled 1.0.3 and fix the process so 1.0.4 and every release after it is correct. Users on 1.0.3 (build `20260522`) will pick up the next update automatically as long as 1.0.4's build number exceeds `20260522` — `release.sh` derives the build number as the UTC `YYYYMMDD`, so any 1.0.4 cut on or after 2026-05-23 satisfies this.

## Mitigation landed (release-tooling guards)

Process hardening so this class of drift cannot recur. No app code changed.

- **`scripts/appcast-item.sh` (new)** — generates the appcast `<item>` from the DMG itself: mounts it, reads `CFBundleShortVersionString` / `CFBundleVersion` from the bundle inside, computes byte length, and runs `sign_update` for the EdDSA signature. Nothing is hand-typed, so `sparkle:version` / `sparkle:shortVersionString` / `length` can no longer drift from the artifact. `release.sh` prints this block at the end of every run.
- **`scripts/release.sh`** — added a version-consistency gate after signature verification: the built bundle's `CFBundleShortVersionString` must equal `MARKETING_VERSION` from `App.xcconfig`, and its `CFBundleVersion` must equal the injected build number. Fails the release before notarization if either drifts. Also emits the generated appcast item at the end.
- **`scripts/preflight.sh`** — added two gates: (1) a marketing-version bump gate that `[✗]`-fails if `MARKETING_VERSION` is not strictly greater than the newest version in the appcast (catches the forgot-to-bump cause); (2) an "Appcast ↔ DMG consistency" gate that mounts the newest item's local DMG and warns if its real version fields or byte length disagree with the advertised attributes. Run against the current repo, the bump gate correctly fails (1.0.3 not yet bumped) and the consistency gate surfaces the known 1.0.3 drift as warnings.
- **`scripts/RELEASE.md` / `scripts/SPARKLE.md`** — per-release steps now point at `scripts/appcast-item.sh` instead of hand-authoring, and document the bump gate.

### Verification

- `zsh -n` clean on all three scripts.
- `scripts/appcast-item.sh website/downloads/Batty-1.0.3.dmg` correctly reports the artifact as `1.0.2` / build `20260522` (demonstrating it reads the real fields).
- `scripts/preflight.sh --skip-build --allow-dirty --skip-ssh-check` — bump gate `[✗]` "MARKETING_VERSION (1.0.3) equals the newest released version", consistency gate `[!]` on both version fields and `[✓]` on length.

Resolved on the basis that the prevention work — not a 1.0.3 re-cut — is the deliverable. The new gates will be exercised for real when 1.0.4 ships.
